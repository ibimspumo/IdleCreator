...
```
Now that `src/components/LogicEditor/nodes` directory is created, I will proceed with **Step 3: Extract individual node components**.

I will do this one by one, reading `CustomNodes.jsx` each time to ensure the `old_string` is accurate.

First, **Extract `EventNode` and `EVENT_TYPES` to `src/components/LogicEditor/nodes/EventNode.jsx`**.

Here's the content for `src/components/LogicEditor/nodes/EventNode.jsx`:
```jsx
import React, { useCallback, useContext } from 'react';
import { Handle, Position } from 'reactflow';
import { GameDataContext } from '../../Editor/GameDataContext'; // Global game data context
