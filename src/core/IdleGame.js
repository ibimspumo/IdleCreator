...
class IdleGame {
     ...
     * @param {Object} config - Konfigurationsobjekt (oder GameTemplate)
     ...
     */
    constructor(config = {}) {
        // Check if config is a GameTemplate
        if (config instanceof GameTemplate) {
            this.template = config;
            this.config = {
                tickRate: config.settings.tickRate || 100,
                saveInterval: config.settings.saveInterval || 5000,
                saveKey: `idleGame_${config.meta.id}_save`
            };
        } else {
        ...
        // Dynamic Renderer (wenn Template vorhanden)
        this.renderer = null;
    ...
    init() {
        ...
        // Wenn Template vorhanden, registriere Upgrades aus Template
        if (this.template) {
    ...
    /**
     * Initialisiert das Spiel aus einem Template
     */
    initializeFromTemplate() {
        if (!this.template) return;

        // Erstelle Renderer
        if (typeof DynamicRenderer !== 'undefined') {
            this.renderer = new DynamicRenderer(this.template, this);
        }

        // Registriere alle Upgrades aus dem Template
        this.template.upgrades.forEach(upgradeData => {
            this.registerUpgrade(upgradeData);
        });

        // Achievements registrieren (später)
        // this.template.achievements.forEach(...)
    }
    ...
    /**
     * Prüft ob ein Achievement erreicht wurde (für später)
     * @param {string} achievementId - ID des Achievements
     * @returns {boolean} True wenn erreicht
     */
    ...
    /**
     * Gibt das Template zurück (falls vorhanden)
     * @returns {GameTemplate|null} Template
     */
    getTemplate() {
    ...
    /**
     * Gibt den Renderer zurück (falls vorhanden)
     * @returns {DynamicRenderer|null} Renderer
     */
    getRenderer() {
